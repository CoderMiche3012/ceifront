...
export default function Menu({ sidebarOpen, onLogout }) {
  ...
  const getLinkClasses = useCallback(
    ({ isActive }) => {
      ...
      const base =
        "flex items-center rounded-2xl py-3 text-[15px] font-medium transition-all duration-200";
  ...
  return (
    <aside className="flex h-screen w-full flex-col border-r border-slate-200 bg-[#f7f7f8] transition-all">
      ...
      <div className="flex items-center gap-3 border-b border-slate-200 px-4 py-5">
        ...
        <div
          className={`${sidebarOpen ? "block" : "hidden"} lg:block min-w-0 animate-in fade-in slide-in-from-left-2`}
        >
          ...
          {visibleMainMenu.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`${sidebarOpen ? "inline" : "hidden"} lg:inline truncate`}
                >
                  {item.label}
                </span>
              ...
              {canViewConfig && (
                ...
                <div className="space-y-1">
                  ...
                  <button
                    ...
                    className={`flex w-full items-center rounded-2xl py-3 text-[15px] font-medium text-[#0E5F63] transition hover:bg-slate-200/50 ${
                      sidebarOpen
                        ? "px-4 gap-3"
                  ...
                  >
                    ...
                    <HiOutlineCog
                      className={`shrink-0 text-xl transition-transform ${
                        configOpen ? "rotate-45" : ""
                      }`}
                    />

                    <div
                      className={`${sidebarOpen ? "flex" : "hidden"} lg:flex flex-1 items-center justify-between`}
                    >
      ...
      <div className="border-t border-slate-200 px-3 py-4 bg-slate-50/50">
        ...
        <button
          onClick={onLogout}
          type="button"
          className={`flex w-full items-center rounded-2xl py-3 text-[15px] font-bold text-red-500 transition-all hover:bg-red-50 active:scale-95 ${
            sidebarOpen
              ? "px-4 gap-3"
              : "justify-center lg:justify-start lg:px-4 lg:gap-3"
          }`}
