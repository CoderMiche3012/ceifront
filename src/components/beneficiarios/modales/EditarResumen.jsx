...
import { actualizarBeneficiario } from '../../../services/beneficiariosService';
...
export default function EditarResumen({ isOpen, onClose, data, setData }) {
  ...
  const handleFinalConfirm = async () => {
    ...
    try {
      ...
      await actualizarBeneficiario(id_expediente, {
        estatus: formDataCache.estatus_beneficiario,
        ...
        <div className="bg-[#F8FAFC] rounded-[2rem] shadow-2xl w-full max-w-3xl overflow-hidden animate-in fade-in zoom-in duration-300 border border-white">
          ...
          <div className="flex items-center justify-between p-8 bg-white border-b border-slate-100">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-2xl font-black text-slate-800 tracking-tight">Editar Datos De Ingreso</h2>
                <p className="text-sm text-slate-500 font-medium">Actualiza la información del beneficiario</p>
              </div>
            </div>
              ...
              <section>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <Field label="Estatus" required>
                    <select
                      name="estatus_beneficiario"
                      defaultValue={data.estatus_beneficiario || ''}
                      required
