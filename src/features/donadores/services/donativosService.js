...
export const obtenerDonativosPorDonador =
  async (idDonador) => {
    const { data } = await API.get(`${BASE_URL}/por-donador/`,
      {
        params: {
          id_donador: idDonador,
        },
...
export const obtenerResumenDonativos =
  async (idPeriodo) => {
    const { data } = await API.get(`${BASE_URL}/resumen/`,
      {
        params: {
          id_periodo: idPeriodo,
        },
...
export const obtenerPeriodosDonativosPorDonador =
  async (idDonador) => {
    const { data } = await API.get(`/api/donadores/donadores/${idDonador}/periodos-donativos/`);
    return data;
  };
