...
select: (res) => {
  ...
  seguimientos
    .filter((s) => String(s.id_periodo) === String(periodoId))
    .forEach((s) => {
  ...
  const beneficiariosProcesados = beneficiarios.map((b) => {
    ...
    return {
      ...
      nombreCompleto: exp
        ? `${exp.nombre} ${exp.apellido_p} ${exp.apellido_m}`.trim().toUpperCase()
        : "SIN NOMBRE",
      id_seguimiento: seguimientoMap[b.id_beneficiario]?.id_seguimiento || null
    };
  });
